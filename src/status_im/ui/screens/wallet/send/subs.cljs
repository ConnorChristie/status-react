                  ...
                  (fn [[{:keys [value to symbol token] :as transaction} contacts balance]]
                    (when transaction
                      (let [contact                 (contacts (utils.hex/normalize-hex to))
                            sufficient-base-funds?  (money/sufficient-funds? value (get balance symbol))
                            sufficient-token-funds? (money/sufficient-funds? (:value token) (get balance (:symbol token)))
                            sufficient-funds?       (and sufficient-base-funds?
                                                         (if (nil? token) true sufficient-token-funds?))]
